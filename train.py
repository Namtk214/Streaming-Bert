...
def evaluate(model, dataloader, device):
    ...
    for batch in dataloader:
        ...
        # Thu thập per-dialogue logits
        for b in range(B):
            turn_logits_np = [
                lg.cpu().numpy() for lg in output["all_turn_logits"][b]
            ]
            all_turn_logits.append(turn_logits_np)
            all_labels.append(batch["labels"][b].item())
            all_num_turns.append(int(batch["turn_mask"][b].sum().item()))

    avg_loss = total_loss / max(num_dialogues, 1)
    metrics = compute_early_exit_metrics(
        all_turn_logits, all_labels, all_num_turns, LABEL_NAMES,
    )
...
def preview_sample(model, dataloader, device, max_samples=2):
    ...
    for preview_idx, dataset_idx in enumerate(selected_indices, start=1):
        ...
        true_label = dialogue["conversation_label"]
        N = item["num_turns"]

        print(f"    Sample {preview_idx}: idx={dataset_idx} "
              f"id={dialogue.get('dialogue_id')} "
              f"label={true_label} ({N} turns)")

        # Show per-turn predictions
        turn_preds = []
        for t, logits_t in enumerate(output["all_turn_logits"][0]):
            ...
            turn_preds.append(pred_name)
            prob_str = " ".join(f"{p:.2f}" for p in probs)
            marker = "✓" if pred_name == true_label else "✗"
            print(f"      Turn {t+1}: {pred_name:10s} [{prob_str}] {marker}")

        final_pred = turn_preds[-1]
        match = "[OK]" if final_pred == true_label else "[X]"
        print(f"      Final: {final_pred} {match}")
