"""
Convert Excel test data → streaming JSON format.
...
Output: JSON file cùng format với train/val/test.json:
  [
    {
      "dialogue_id":        "scam-1",
...
_DEFAULT_OUT = os.path.join(cfg.streaming_data_dir, "excel_test.json")
...
# ── Word segmenter (reuses VNCORENLP_CACHE từ prepare_data) ─────
...
def convert_conversation(label: str, idx, raw_turns: list, segmenter: WordSegmenter) -> dict:
    turn_label = LABEL_MAP[label]
    turns = []
    for i, raw_text in enumerate(raw_turns):
        text = clean_text(raw_text)
        # Alternating speaker: even turn index → người gọi (0), odd → người nghe (1)
...
def main():
    parser = argparse.ArgumentParser(description="Convert Excel test data to streaming JSON.")
    ...
    print("=" * 60)
    print("CONVERT EXCEL → STREAMING JSON")
    print("=" * 60)
    ...
    for t in s["turns"][:3]:
        print(f"  T{t['turn_id']} spk={t['speaker']}: {t['text'][:60]}")
    print("\nDone!")
